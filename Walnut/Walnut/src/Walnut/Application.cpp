...
VkQueue Walnut::Application::GetQueue()
{
	return g_Queue;
}

uint32_t Walnut::Application::GetQueueFamily()
{
...
void check_vk_result(VkResult err)
{
	if (err == 0)
		return;
	//fprintf(stderr, "[vulkan] Error: VkResultss = %d\n", err);
	if (err < 0)
		abort();
}
...
namespace Walnut {
	...
	{
		const uint64_t DEFAULT_FENCE_TIMEOUT = 0;
